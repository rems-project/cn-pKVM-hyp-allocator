...
Analyzes trial-level data from pbt_bug_finding_data.csv and generates statistics.
"""
...
def analyze_trials(trial_data: list[dict]) -> tuple[list[dict], dict]:
    ...
    for (patch_path, function), trials in sorted(patch_groups.items()):
        ...
        success_times = [float(t['elapsed_time']) for t in trials if t['bug_found'] == 'True']

        # Runs and discards for successful detections
        runs_data = [int(t['num_runs']) for t in trials
                     if t['bug_found'] == 'True' and t['num_runs'] != '']
        discards_data = [int(t['num_discards']) for t in trials
                        if t['bug_found'] == 'True' and t['num_discards'] != '']
    ...
    overall_detection_rate = total_bugs_found / total_trials if total_trials > 0 else 0.0

    # Overall time statistics (across all successful detections)
    all_success_times = [float(t['elapsed_time']) for t in trial_data if t['bug_found'] == 'True']
...
def print_report(patch_stats: list[dict], overall_stats: dict):
    """
    ...
    for stat in patch_stats:
        ...
        if not math.isnan(stat['avg_time']):
            print(f"  Avg time:        {stat['avg_time']:.2f}s (min: {stat['min_time']:.2f}s, max: {stat['max_time']:.2f}s)")

        if not math.isnan(stat['avg_runs']):
            print(f"  Avg runs:        {stat['avg_runs']:.1f} (min: {stat['min_runs']}, max: {stat['max_runs']})")

        if not math.isnan(stat['avg_discards']):
            print(f"  Avg discards:    {stat['avg_discards']:.1f} (min: {stat['min_discards']}, max: {stat['max_discards']})")
...
def main():
    ...
    # Load trial data
    trial_data = load_trial_data('pbt_bug_finding_data.csv')
    ...
    # Print report
    print_report(patch_stats, overall_stats)

    # Save summary CSV
    save_summary_csv(patch_stats)
